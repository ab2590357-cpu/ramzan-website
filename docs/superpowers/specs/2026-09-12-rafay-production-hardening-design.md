...
2. **Storage reliability:** public media upload, site-data reads/writes, and private booking records use different storage paths. Public Vercel Blob operations currently rely on implicit environment behavior and treat `BLOB_STORE_ID` as sufficient configuration. Current Vercel Blob documentation describes the project read-write token as the stable SDK credential. All storage calls should use explicit, testable credentials and return structured failures.
...
- Make public Vercel Blob authentication deterministic by using `BLOB_READ_WRITE_TOKEN` explicitly.
...
### Public Blob credentials

`RAFAY_DATA_DIR` remains highest priority. Otherwise public site config/media require `BLOB_READ_WRITE_TOKEN`; `BLOB_STORE_ID` alone is not treated as writable configuration. The public token is passed explicitly to `head`, `get`, `put`, `list`, and `del`. Private booking operations continue to pass `RAFAY_PRIVATE_BLOB_READ_WRITE_TOKEN` explicitly.
...
Storage fallback paths log short structured messages containing operation and error name/message, never credentials or admin secrets. Health reporting and logs allow deployment/storage problems to be distinguished without patching blindly.
...
- Public Blob code never considers only `BLOB_STORE_ID` sufficient for write access.
- Public Blob calls pass the explicit public read-write token when the filesystem backend is not active.
