...
export function AdminClient({ secret, initialData, initialEtag }: Props) {
  ...
  const moveProfile = (id: string, direction: -1|1) => {
    const items = [...draft.profiles].sort((a,b)=>a.displayOrder-b.displayOrder); const index = items.findIndex((item)=>item.id===id); const target=index+direction; if(index<0||target<0||target>=items.length)return; [items[index],items[target]]=[items[target],items[index]]; patch({profiles:normalizeOrders(items)});
  };
  ...
  return <AdminShell view={view} onViewChange={setView} saveState={saveState} onSave={save} message={message}>
    {view === 'overview' && <section className={styles.panel}><span className="rafay-kicker">Overview</span><h1 className="rafay-display">RAFAY Control Center</h1><p className={styles.lead}>Everything important on the public booking experience is controlled here. No public admin link is exposed.</p><div className={styles.statGrid}><Stat label="Active profiles" value={String(activeProfiles)} /><Stat label="Active packages" value={String(activePackages)} /><Stat label="Payment methods" value={String(draft.paymentMethods.filter((p)=>p.active).length)} /><Stat label="Config version" value={`v${draft.version}`} /></div><div className={styles.notice}>Media and booking-management modules are connected in the next implementation tasks. Current content edits already use shared server data.</div></section>}
    ...
    {view === 'website-content' && <ContentEditor data={draft} onChange={patch} />}
    ...
    {view === 'bookings' && <section className={styles.panel}><Header eyebrow="Requests" title="Bookings"/><div className={styles.emptyModule}>Booking list and status controls are connected in Task 8.</div></section>}
    {view === 'media' && <section className={styles.panel}><Header eyebrow="Assets" title="Media"/><div className={styles.emptyModule}>Profile and website image uploads are connected in Task 7.</div></section>}
  </AdminShell>;
}
