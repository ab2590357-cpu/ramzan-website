...
export function AdminClient({ secret, initialData, initialEtag }: Props) {
  ...
  return <AdminShell view={view} onViewChange={setView} saveState={saveState} onSave={save} message={message}>
    {view === 'overview' && <section className={styles.panel}><span className="rafay-kicker">Overview</span><h1 className="rafay-display">RAFAY Control Center</h1><p className={styles.lead}>Everything important on the public booking experience is controlled here. No public admin link is exposed.</p><div className={styles.statGrid}><Stat label="Active profiles" value={String(activeProfiles)} /><Stat label="Active packages" value={String(activePackages)} /><Stat label="Payment methods" value={String(draft.paymentMethods.filter((p)=>p.active).length)} /><Stat label="Config version" value={`v${draft.version}`} /></div><div className={styles.notice}>Profile media uploads are connected. Booking request management is the next control-center module.</div></section>}
